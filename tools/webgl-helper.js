...
function createProgram(gl, vertexShader, fragmentShader) {
	...
	vertexShader && gl.attachShader(program, vertexShader); // 应用顶点着色器
	fragmentShader && gl.attachShader(program, fragmentShader); // 应用片元着色器
...
function loadTexture(gl, src, attribute, callback) {
	...
	img.onload = function () {
		gl.activeTexture(gl.TEXTURE0); // 默认激活通道0的纹理，故也可以不显式激活。
		let texture = gl.createTexture(); //  创建纹理对象
		gl.bindTexture(gl.TEXTURE_2D, texture); // 绑定纹理对象到当前
		gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, img); // 为片元着色器传递数据
		gl.texParameterf(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR); // 设置放大缩小所采用的算法
		gl.texParameterf(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
		gl.uniform1i(attribute, 0); // 0对应TEXURE0中的通道号
		callback && callback();
	};
	img.src = src;
}
